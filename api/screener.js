 ...
 * api/screener.js  v3
 * 全市場選股篩選 API
 * 資料來源：TWSE（完全免費，不需要 Token）
 * 改用「指定日期」端點，非交易時間也可正常使用
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`[screener] Using trade date: ${tradeDate}`);

    // 並行拉取三個 TWSE 資料集
    const [bwiResult, dayResult, instResult] = await Promise.allSettled([
      fetchTWSE(`${TWSE_RWD}/afterTrading/BWIBBU_d?date=${tradeDate}&response=json`),   // 殖利率/本益比
      fetchTWSE(`${TWSE_RWD}/afterTrading/MI_INDEX?date=${tradeDate}&response=json`),    // 大盤（備用）
      fetchTWSE(`${TWSE_RWD}/fund/T86?date=${tradeDate}&selectType=ALL&response=json`),  // 三大法人個股
    ]);

    const bwiRaw  = bwiResult.status  === 'fulfilled' ? bwiResult.value  : null;
    const instRaw = instResult.status === 'fulfilled' ? instResult.value : null;

    console.log(`[screener] BWI status:${bwiResult.status} rows:${bwiRaw?.data?.length ?? 0}`);
    console.log(`[screener] Inst status:${instResult.status} rows:${instRaw?.data?.length ?? 0}`);

    if (!bwiRaw?.data || bwiRaw.data.length === 0) {
      throw new Error(`TWSE BWIBBU_d 無資料 (date=${tradeDate})，請稍後再試`);
    }

    const stocks = buildStocks(bwiRaw, instRaw, tradeDate);
...
async function getLastTradeDate() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // 嘗試取當日資料，若有資料就用這天
    try {
      const resp = await fetchTWSE(
        `${TWSE_RWD}/afterTrading/BWIBBU_d?date=${dateStr}&response=json`
      );
      if (resp?.data && resp.data.length > 0) return dateStr;
    } catch {}
  }
  // 備援：用固定的最近已知交易日
  return '20260522';
...
// ── 整合資料 ──────────────────────────────────────────────────────
function buildStocks(bwiRaw, instRaw, tradeDate) {
  // ── BWI 資料 ──────────────────────────────────────────────────
  // TWSE BWIBBU_d 欄位順序（fields 陣列）：
  // 0:證券代號 1:證券名稱 2:本益比 3:股利年度 4:殖利率(%) 5:股價淨值比
  const bwiFields = bwiRaw.fields || [];
  const codeIdx   = findIdx(bwiFields, ['證券代號','Code']);
  const nameIdx   = findIdx(bwiFields, ['證券名稱','Name']);
  const peIdx     = findIdx(bwiFields, ['本益比','PERatio']);
  const yieldIdx  = findIdx(bwiFields, ['殖利率(%)','DividendYield']);
  const pbIdx     = findIdx(bwiFields, ['股價淨值比','PBRatio']);

  // ── 法人資料 ──────────────────────────────────────────────────
  // TWSE T86 欄位：0:證券代號 1:證券名稱 ... 外資買 賣 淨 投信買 賣 淨 自營買 賣 淨
  const instMap = {};
  if (instRaw?.data) {
    const iFields = instRaw.fields || [];
    const iCodeIdx = findIdx(iFields, ['證券代號','Code']);
    // 找外資買賣超和投信買賣超欄位
    const fNetIdx  = findIdx(iFields, ['外陸資買賣超股數(千股)','外資買賣超']);
    const tNetIdx  = findIdx(iFields, ['投信買賣超股數(千股)','投信買賣超']);

    for (const row of instRaw.data) {
      const code = iCodeIdx >= 0 ? row[iCodeIdx]?.trim() : null;
      if (!code || !/^\d{4}$/.test(code)) continue;
      const fNet = parseInt((fNetIdx >= 0 ? row[fNetIdx] : '0').replace(/,/g, '')) || 0;
      const tNet = parseInt((tNetIdx >= 0 ? row[tNetIdx] : '0').replace(/,/g, '')) || 0;
      instMap[code] = { foreignNet: fNet, trustNet: tNet };
    }
  }
  ...
  return (bwiRaw.data || [])
    .map(row => {
      ...
      const name  = (nameIdx >= 0 ? row[nameIdx] : row[1])?.trim() || code;
      const pe    = parseFloat((peIdx    >= 0 ? row[peIdx]    : row[2]) || '0') || 0;
      const yield_= parseFloat((yieldIdx >= 0 ? row[yieldIdx] : row[4]) || '0') || 0;
      const pb    = parseFloat((pbIdx    >= 0 ? row[pbIdx]    : row[5]) || '0') || 0;

      // 估算收盤價（需要股價資料，暫用 EPS*PE 估算）
      // TWSE BWIBBU_d 本身不含收盤價，用 STOCK_DAY 補
      // 這裡先用 pb 和 pe 估算
      const roe   = (pb > 0 && pe > 0) ? +(pb / pe * 100).toFixed(1) : 8;
      // 實際收盤價無法從此 API 直接取得，先填 0，稍後補
      const close = 0;
      const eps   = 0;
    ...
    .filter(s => s && s.yield_ > 0); // 過濾無效資料
}

// ── 找欄位 index ─────────────────────────────────────────────────
function findIdx(fields, names) {
  for (const name of names) {
    const idx = fields.findIndex(f => f && f.includes(name));
    if (idx >= 0) return idx;
  }
  return -1;
}
